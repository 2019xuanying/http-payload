...
# WSS_CONFIG_BLOCK_START -- managed by deploy_wss_final.sh
...
# WSS_CONFIG_BLOCK_END -- managed by deploy_wss_final.sh
...
# 1. 部署 WSS 面板 Python 脚本 (/usr/local/bin/wss_panel.py)
echo "==== 部署 WSS Web 面板脚本 ===="
...
def format_bytes(bytes_value):
    ...
    if bytes_value < 1048576: # Less than 1MB
        return f"{bytes_value / 1024:.2f} KB"
    elif bytes_value < 1073741824: # Less than 1GB
                ...
                <div class="card p-6">
                    ...
                    <form method="POST" action="{{ url_for('add_user') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <p class="text-xs text-gray-500 mt-1">留空则为永久有效。</p>
                        </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                    <form method="POST" action="{{ url_for('delete_user') }}" onsubmit="return confirm('确认删除用户 {{ user.username }} 吗？')">
...
def add_user():
    ...
    try:
        ...
        # 移除 sudo 权限
        if run_cmd(['gpasswd', '-d', username, 'sudo']): pass
...
def delete_user():
    ...
    try:
        # -r 删除 home 目录
        subprocess.run(['userdel', '-r', username], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except subprocess.CalledProcessError as e:
        # 即使删除系统用户失败，也要从 JSON 中删除，避免面板管理混乱
        pass 
    ...
    if users_before != users_after:
        save_users(users)
        return redirect(url_for('dashboard', message=f"用户 {username} 删除成功!"))
    else:
        return redirect(url_for('dashboard', message=f"用户 {username} 未找到或删除失败!"))

# if __name__ == '__main__':
#     app.run(host='0.0.0.0', port=PANEL_PORT, debug=False)
EOF

# 2. 部署 WSS 流量统计和清理脚本 (核心修复: 纯 Python)
echo "==== 部署 WSS 流量统计脚本 (核心修复) ===="
...
def update_traffic_and_check_expiration():
    """Updates simulated traffic and checks expiration status."""
    users = load_users()
    ...
    for user in users:
        ...
        # 2. Execute traffic simulation (only for active users)
        if user['status'] == 'active':
            current_usage = user.get('usage_bytes', 0)
            
            if current_usage < SIMULATION_CAP:
                # Accumulate simulated traffic
...
def cleanup_expired_users():
    """Deletes system accounts for users marked as 'expired'."""
    users = load_users()
    users_to_keep = []
    ...
    for user in users:
        if user['status'] == 'expired':
            ...
            # 删除系统账户 (-r removes home directory)
