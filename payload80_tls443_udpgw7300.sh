...
BUFLEN = 2048 * 4
TIMEOUT = 60
DEFAULT_HOST = '127.0.0.1:##SSH_PORT##' # 占位符将被替换

# HTTP 101 Switching Protocols 响应，用于模拟 WSS 握手
...
class ConnectionHandler(threading.Thread):
    ...
    def run(self):
        try:
            ...
            if len(PASS) and passwd != PASS:
                self.client.send(b'HTTP/1.1 200 WrongPass!\r\n\r\n')
                return
...
# 步骤 2: 使用 sed 注入 $SSH_PORT 变量，并移动到最终位置
sudo sed "s/##SSH_PORT##/$SSH_PORT/g" /usr/local/bin/wss_temp_py | sudo tee /usr/local/bin/wss > /dev/null
sudo rm /usr/local/bin/wss_temp_py
